function Animal()
{
	...
	var list = LGlobal.divideCoordinate(491,86, 1,10);//小人图片像素
    ...
    var data = new LBitmapData(imglist["rabbit"],0 ,0 ,40,85);
...
Animal.prototype.run = function()
{
	...
	for(i in floorlayer.childList)//判断小人是否在木板上
	{
		...
		if(this.speed>0&&(this.y+86>=floorlayer.childList[i].y&&this.y-floorlayer.childList[i].y<=10)&&
		(this.x-floorlayer.childList[i].x<=100&&floorlayer.childList[i].x-this.x<=30))
		{
			...
			this.y = floorlayer.childList[i].y-86;//让小人站在木板上小人像素高为50
	...
	for(j in BlackHolelayer.childList)
	{
		...
		if((this.x-BlackHolelayer.childList[j].x<=40&&BlackHolelayer.childList[j].x-this.x<=40)&&(BlackHolelayer.childList[j].y-this.y<=86&&this.y-BlackHolelayer.childList[j].y<=86))
		{
	...
	for(k in foodlayer.childList)
	{
		...
		if((this.x-foodlayer.childList[k].x<=50&&foodlayer.childList[k].x-this.x<=50)&&(foodlayer.childList[k].y-this.y<=70&&this.y-foodlayer.childList[k].y<=70))
		{
			...
			// sound.stop();
			foodnumber+=1;
			carrotText.text=": " + foodnumber;

			if(foodnumber >= 5)
			{
				if(rabbit.hp!=this.maxhp)
				{
					var myTimer = new LTimer(1000, 1);
					myTimer.start();
					addhealth=new LBitmap(new LBitmapData(imglist["addHealth"]));
					addhealth.y=280;
					addhealth.x=240;
					addhealth.alpha = 0.9;
					bglayer.addChild(addhealth);
					health+=46;
