function Animal()
{
    ...
    var data=new LBitmapData(imglist["rabbit"],0 ,0 ,20 ,80);
...
Animal.prototype.run=function()
{
	...
	for(i in floorlayer.childList)
	{
		...
		//
		if(this.speed>0&&(this.y+85>=floorlayer.childList[i].y&&this.y-floorlayer.childList[i].y<=10)&&
	...
	for(j in foodlayer.childList)
	{
		...
		if((this.x-foodlayer.childList[j].x<=40&&foodlayer.childList[j].x-this.x<=40)&&(foodlayer.childList[j].y-this.y<=80&&this.y-foodlayer.childList[j].y<=80))
		{
...
function onkeydown(event)
{
	...
	else if(event.keyCode==68)//D从天而降
	{
		if(rabbit.mp>0)
		{
			...
			rabbit.y=floorlayer.childList[i].y-80;
